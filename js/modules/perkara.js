...
import { escText, getVal, formatDate, formatDateIndo, formatModusVerb, terbilangHuruf, autoGenerateTeksTanggalBA} from '../utils/formatters.js';
import { showToast, showToastConfirm } from './ui.js';
import { refreshReportTableUI, updateReportLive } from './editor.js';

let currentPerkaraPage = 1;
const rowsPerPagePerkara = 5;
...
export function performLPValidation() {
  ...
  if (defaultTglBA) {
    autoGenerateTeksTanggalBA(defaultTglBA);
  } else {
...
export function renderPerkaraTable() {
  ...
  let totalPages = Math.ceil(filtered.length / rowsPerPagePerkara) || 1;
  if (currentPerkaraPage > totalPages) currentPerkaraPage = totalPages;
  if (currentPerkaraPage < 1) currentPerkaraPage = 1;

  let startIdx = (currentPerkaraPage - 1) * rowsPerPagePerkara;
  let paginatedData = filtered.slice(startIdx, startIdx + rowsPerPagePerkara);
  // ---------------------------------------------
  ...
  if (paginatedData.length === 0) {
    ...
    // Perbarui teks info & nonaktifkan tombol navigasi jika kosong
    let pageInfo = document.getElementById('perkaraPageInfo');
    if (pageInfo) pageInfo.textContent = `Menampilkan 0 data`;
    
    let btnPrev = document.getElementById('btnPrevPerkara');
    let btnNext = document.getElementById('btnNextPerkara');
    if (btnPrev) btnPrev.disabled = true;
    if (btnNext) btnNext.disabled = true;
    return;
  }

  tbody.innerHTML = paginatedData.map((rec, index) => {
    let i = startIdx + index; // Nomor urut konsisten melanjutkan halaman
    let originalIndex = filtered.indexOf(rec); // Indeks asli untuk fungsi edit/hapus
    ...
    let actionButtons = `
      <button type="button" class="btn primary" style="padding:2px 6px; font-size:10px;" onclick="editPerkara(${originalIndex})">
        <i class="fi fi-rr-edit"></i> Edit
      </button>
      <button type="button" class="btn danger" style="padding:2px 6px; font-size:10px;" onclick="deletePerkara(${originalIndex})">
  ...
  let pageInfo = document.getElementById('perkaraPageInfo');
  if (pageInfo) {
    pageInfo.textContent = `Halaman ${currentPerkaraPage} dari ${totalPages} (Total: ${filtered.length} data)`;
  }
  
  let btnPrev = document.getElementById('btnPrevPerkara');
  let btnNext = document.getElementById('btnNextPerkara');
  if (btnPrev) btnPrev.disabled = currentPerkaraPage === 1;
  if (btnNext) btnNext.disabled = currentPerkaraPage === totalPages;
}
...
export function changePerkaraPage(direction) {
  currentPerkaraPage += direction;
  renderPerkaraTable(); // Sesuaikan dengan nama fungsi render tabel perkara Anda
}